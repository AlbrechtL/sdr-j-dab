 ...
 *	sdr-j dab decoder(s) (with removal of the erasure facility,
 *	since in DAB we do not have that
 */
...
//
//	for DAB+ frames, the parameters are 8, 0x11d, 0, 1, 10
...
DTYPE lambda [rsHandle -> nroots + 1],
      syndrome [rsHandle -> nroots];	/* Err+Eras Locator poly
					 * and syndrome poly */
	...
	for (j = 1; j < rsHandle -> nn; j++){
	   for (i = 0; i < rsHandle -> nroots; i++){
	      if (syndrome [i] == 0){
	         ...
	         syndrome [i] = data [j] ^
	                     rsHandle -> alpha_to [modnn (rsHandle,
	                                                  rsHandle -> index_of [syndrome [i]] + (rsHandle -> fcr + i) * rsHandle -> prim)];
	      }
	   }
 	}
...
/*	Convert syndromes to index form, checking for nonzero condition */
	...
	for (i = 0; i < rsHandle -> nroots; i++){
	   ...
	   syndrome [i] = rsHandle -> index_of [syndrome [i]];
	}
...
//
//	OK, here we know there are errors
...
/*	Compute discrepancy at the r-th step in poly-form */
	   ...
	   for (i = 0; i < r; i++) {
	      if ((lambda [i] != 0) && 
	          (syndrome [r-i-1] != rsHandle -> nn)) {
	         discr_r ^= rsHandle -> alpha_to [modnn (rsHandle,
	                                                 rsHandle -> index_of [lambda[i]] + syndrome [r-i-1])];
	      }
	   }
